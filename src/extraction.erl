...
extract_links(Url, Body) ->
    ...
    AsBinaries = [list_to_binary(X) || X<-GoodLinks],
    ordsets:to_list(ordsets:from_list(AsBinaries)).

-ifdef(TEST).
extract_links_test_() ->
    Url="http://www.example.com/abc/dec",
    [?_assertEqual([<<"http://www.example.com/abc/link1">>, <<"http://www.example.com/background.png">>, <<"http://www.foobar.com/pixel.gif">>],
		  extract_links(Url, "HREF=\"link1\" href=\"mailto:foo@bar.com\" <img src=\"http://www.foobar.com/pixel.gif\" /> <body background=\"/background.png\"")),
